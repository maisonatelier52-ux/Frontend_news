...
export default function RightOfReplyPolicyManagerPage() {
  ...
  async function fetchSettings() {
    try {
      ...
      console.error('Failed to load right of reply policy settings', e);
    } finally {
      setLoading(false);
    }
  }
    ...
    try {
      ...
      if (res.ok) {
        ...
        await fetch('/api/logs', {
          ...
          body: JSON.stringify({
            type: 'activity',
            action: 'RIGHT_OF_REPLY_POLICY_UPDATE',
            details: { title },
            user: 'Admin'
          })
        });
  ...
  return (
    <div className="max-w-[1150px] animate-[admin-fade-in_0.4s_ease_both] pb-12">
      <div className="flex justify-between items-start mb-6">
        <div>
          <h1 className="text-[22px] font-sans font-extrabold text-[#111] m-0">Right of Reply Policy Manager</h1>
          <p className="text-[12.5px] text-slate-500 mt-1 font-medium">Manage and document fair reporting opportunities, post-publication response requests, and contact details.</p>
        </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-[0_4px_20px_rgba(0,0,0,0.01)] space-y-4">
            <div className="flex justify-between items-center">
              <span className="text-[11px] font-bold text-slate-400 uppercase tracking-widest block">Policy Sections & Standards</span>
                        ...
                        <div className="space-y-2 pt-1">
                          {sec.listItems.map((item, itemIdx) => (
                            <div key={itemIdx} className="flex gap-2 items-center">
                              ...
                              <button onClick={() => handleRemoveListItem(secIdx, itemIdx)} className="text-red-505 hover:text-red-755 font-bold text-xs cursor-pointer border-none bg-transparent">
                                ✕
                              </button>
                            </div>
                          ))}
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-[0_4px_20px_rgba(0,0,0,0.01)] sticky top-6 space-y-4">
            ...
            <div className="border border-slate-150 rounded-lg p-4 bg-white font-sans max-h-[600px] overflow-y-auto space-y-4 select-none">
              <div className="pb-3 border-b border-slate-100">
                <h2 className="font-serif text-lg font-bold text-slate-900 leading-tight">{title || 'Right of Reply Policy'}</h2>
                <p className="text-[9.5px] font-bold text-slate-400 uppercase mt-0.5 tracking-wider">{subtitle || 'Opportunity for fair response'}</p>
              </div>
