...
export default function LegalPolicyManagerPage() {
  ...
  async function fetchSettings() {
    try {
      ...
      if (res.ok) {
        ...
        if (data.legalPolicy) {
          const l = data.legalPolicy;
          setTitle(l.title || '');
          setSubtitle(l.subtitle || '');
          setLeadParagraph(l.leadParagraph || '');
          setSections(l.sections || []);
        }
      }
    } catch (e) {
      console.error('Failed to load legal policy settings', e);
    } finally {
      setLoading(false);
    }
  }
  ...
  function handleResetOriginal() {
    showConfirm(
      ...
      async () => {
        try {
          setLoading(true);
          const res = await fetch('/api/settings/defaults?key=legalPolicy');
          if (res.ok) {
            const data = await res.json();
            if (data.legalPolicy) {
              const l = data.legalPolicy;
  ...
  async function handleSave() {
    try {
      ...
      if (res.ok) {
        setSaved(true);
        showAlert('Legal Policy updated successfully!', 'success', 'Saved');
        ...
        showAlert('Failed to save Legal Policy settings', 'error', 'Error');
      }
    } catch (err) {
      showAlert('Failed to save Legal Policy settings', 'error', 'Error');
    }
  ...
  return (
    <div className="max-w-[1150px] animate-[admin-fade-in_0.4s_ease_both] pb-12">
      <div className="flex justify-between items-start mb-6">
        <div>
          <h1 className="text-[22px] font-sans font-extrabold text-[#111] m-0">Legal Policy Manager</h1>
          <p className="text-[12.5px] text-slate-500 mt-1 font-medium">Configure governing laws, trademark notices, endorsements disclaimers, and liabilities terms.</p>
        </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-[0_4px_20px_rgba(0,0,0,0.01)] space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <label className="text-[11px] font-bold text-slate-650">Page Title</label>
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-[0_4px_20px_rgba(0,0,0,0.01)] space-y-4">
            <div className="flex justify-between items-center">
              <span className="text-[11px] font-bold text-slate-400 uppercase tracking-widest block">Legal Disclaimers & Rules</span>
            ...
            {sections.length === 0 ? (
              <div className="text-center text-slate-400 text-xs py-8 font-sans border border-dashed border-slate-200 rounded-lg">
                No policy sections added yet. Click "+ Add Section".
              </div>
            ) : (
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-[0_4px_20px_rgba(0,0,0,0.01)] sticky top-6 space-y-4">
            ...
            <div className="border border-slate-150 rounded-lg p-4 bg-white font-sans max-h-[600px] overflow-y-auto space-y-4 select-none">
              <div className="pb-3 border-b border-slate-100">
                <h2 className="font-serif text-lg font-bold text-slate-900 leading-tight">{title || 'Legal Policy'}</h2>
                <p className="text-[9.5px] font-bold text-slate-400 uppercase mt-0.5 tracking-wider">{subtitle || 'Governing Laws and Disclaimers'}</p>
              </div>
