...
export default function OwnershipFundingManagerPage() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.ownershipFunding) {
          const o = data.ownershipFunding;
          setTitle(o.title || '');
          setSubtitle(o.subtitle || '');
          setLeadParagraph(o.leadParagraph || '');
          setSections(o.sections || []);
        }
      }
    } catch (e) {
      console.error('Failed to load ownership settings', e);
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleResetOriginal() {
    ...
    try {
      setLoading(true);
      const res = await fetch('/api/settings/defaults?key=ownershipFunding');
      if (res.ok) {
        const data = await res.json();
        if (data.ownershipFunding) {
          const o = data.ownershipFunding;
    ...
    try {
      ...
      if (res.ok) {
        ...
        await fetch('/api/logs', {
          ...
          body: JSON.stringify({
            type: 'activity',
            action: 'OWNERSHIP_FUNDING_UPDATE',
            details: { title },
            user: 'Admin'
          })
        });
      ...
      alert('Failed to save Ownership & Funding settings');
    }
  }
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[300px] gap-2">
        ...
        <span className="text-xs font-semibold text-slate-500 font-sans">Syncing Ownership & Funding configuration...</span>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-[1150px] animate-[admin-fade-in_0.4s_ease_both] pb-12">
      <div className="flex justify-between items-start mb-6">
        <div>
          <h1 className="text-[22px] font-sans font-extrabold text-[#111] m-0">Ownership & Funding Manager</h1>
          <p className="text-[12.5px] text-slate-500 mt-1 font-medium">Manage and document company owner cooperatives, grant details, and financial disclosures.</p>
        </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-[0_4px_20px_rgba(0,0,0,0.01)] space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-1.5">
                <label className="text-[11px] font-bold text-slate-655">Subtitle / Description</label>
                <input value={subtitle} onChange={(e) => setSubtitle(e.target.value)} style={{ ...inputStyle, width: '100%' }} />
              </div>
            </div>

            <div className="space-y-1.5">
              <label className="text-[11px] font-bold text-slate-655">Lead Paragraph Text (use **Text** for bold highlight)</label>
              <textarea 
                rows={3}
                value={leadParagraph} 
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-[0_4px_20px_rgba(0,0,0,0.01)] space-y-4">
            <div className="flex justify-between items-center">
              <span className="text-[11px] font-bold text-slate-400 uppercase tracking-widest block">Ownership & Revenue Clauses</span>
              ...
              <div className="space-y-6 pt-2">
                {sections.map((sec, secIdx) => (
                  <div key={secIdx} className="border border-slate-200 rounded-lg p-4 bg-slate-50/50 space-y-4 relative">
                    <button onClick={() => handleRemoveSection(secIdx)} className="absolute top-3 right-3 text-red-500 hover:text-red-755 font-bold text-xs bg-transparent border-none cursor-pointer">
                      ✕ Delete Section
                    </button>
                        ...
                        <div className="space-y-2 pt-1">
                          {sec.listItems.map((item, itemIdx) => (
                            <div key={itemIdx} className="flex gap-2 items-center">
                              ...
                              <button onClick={() => handleRemoveListItem(secIdx, itemIdx)} className="text-red-500 hover:text-red-755 font-bold text-xs cursor-pointer border-none bg-transparent">
                                ✕
                              </button>
                            </div>
                          ))}
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-[0_4px_20px_rgba(0,0,0,0.01)] sticky top-6 space-y-4">
            ...
            <div className="border border-slate-150 rounded-lg p-4 bg-white font-sans max-h-[600px] overflow-y-auto space-y-4 select-none">
              <div className="pb-3 border-b border-slate-100">
                <h2 className="font-serif text-lg font-bold text-slate-900 leading-tight">{title || 'Ownership & Funding'}</h2>
                <p className="text-[9.5px] font-bold text-slate-400 uppercase mt-0.5 tracking-wider">{subtitle || 'Editorial Independence through Transparent Revenue'}</p>
              </div>
              ...
              {sections.map((sec, idx) => (
                <div key={idx} className="space-y-2 mt-4">
                  ...
                  <p className="text-[11px] text-slate-505 leading-relaxed whitespace-pre-line m-0">{sec.content.replace(/\*\*/g, '')}</p>
                  
                  {sec.listItems && sec.listItems.length > 0 && (
                    <ul className="list-disc pl-4 space-y-1 text-[11px] text-slate-550">
                      {sec.listItems.map((item, itemIdx) => (
                        <li key={itemIdx}>{item.replace(/\*\*/g, '')}</li>
                      ))}
                    </ul>
                  )}
                </div>
