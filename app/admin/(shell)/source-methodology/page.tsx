...
export default function SourceMethodologyManagerPage() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.sourceMethodology) {
          const s = data.sourceMethodology;
          setTitle(s.title || '');
          setSubtitle(s.subtitle || '');
          setLeadParagraph(s.leadParagraph || '');
          setSections(s.sections || []);
        }
      }
    } catch (e) {
      console.error('Failed to load source methodology settings', e);
    } finally {
      setLoading(false);
    }
  }
  ...
  function handleResetOriginal() {
    showConfirm(
      ...
      async () => {
        try {
          setLoading(true);
          const res = await fetch('/api/settings/defaults?key=sourceMethodology');
          if (res.ok) {
            const data = await res.json();
            if (data.sourceMethodology) {
              const s = data.sourceMethodology;
  ...
  function handleSectionChange(index: number, field: keyof MethodologySection, value: any) {
  ...
  async function handleSave() {
    try {
      ...
      if (res.ok) {
        setSaved(true);
        showAlert('Source & Methodology page updated successfully!', 'success', 'Saved');
        ...
        await fetch('/api/logs', {
          ...
          body: JSON.stringify({
            type: 'activity',
            action: 'SOURCE_METHODOLOGY_UPDATE',
            details: { title },
            user: 'Admin'
          })
        });
      } else {
        showAlert('Failed to save Source Methodology settings', 'error', 'Error');
      }
    } catch (err) {
      showAlert('Failed to save Source Methodology settings', 'error', 'Error');
    }
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[300px] gap-2">
        ...
        {/* <span className="text-xs font-semibold text-slate-500 font-sans">Syncing Source Methodology configuration...</span> */}
      </div>
    );
  }
  ...
  return (
    <div className="max-w-[1150px] animate-[admin-fade-in_0.4s_ease_both] pb-12">
      <div className="flex justify-between items-start mb-6">
        <div>
          <h1 className="text-[22px] font-sans font-extrabold text-[#111] m-0">Source Methodology Manager</h1>
          <p className="text-[12.5px] text-slate-500 mt-1 font-medium">Configure vetting processes, fact-checking workflows, and anonymous sources terms.</p>
        </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-[0_4px_20px_rgba(0,0,0,0.01)] space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-1.5">
                <label className="text-[11px] font-bold text-slate-600">Subtitle / Description</label>
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-[0_4px_20px_rgba(0,0,0,0.01)] space-y-4">
            <div className="flex justify-between items-center">
              <span className="text-[11px] font-bold text-slate-400 uppercase tracking-widest block">Methodology Sections</span>
            ...
            {sections.length === 0 ? (
              <div className="text-center text-slate-400 text-xs py-8 font-sans border border-dashed border-slate-200 rounded-lg">
                No methodology sections added yet. Click "+ Add Section".
              </div>
            ) : (
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-[0_4px_20px_rgba(0,0,0,0.01)] sticky top-6 space-y-4">
            ...
            <div className="border border-slate-150 rounded-lg p-4 bg-white font-sans max-h-[600px] overflow-y-auto space-y-4 select-none">
              <div className="pb-3 border-b border-slate-100">
                <h2 className="font-serif text-lg font-bold text-slate-900 leading-tight">{title || 'Source Methodology'}</h2>
                <p className="text-[9.5px] font-bold text-slate-400 uppercase mt-0.5 tracking-wider">{subtitle || 'How we gather and verify information'}</p>
              </div>
