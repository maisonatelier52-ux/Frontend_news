...
export default function CorrectionPolicyManagerPage() {
  ...
  async function fetchSettings() {
    try {
      ...
      console.error('Failed to load correction policy settings', e);
    } finally {
      setLoading(false);
    }
  }
    ...
    try {
      ...
      if (res.ok) {
        ...
        await fetch('/api/logs', {
          ...
          body: JSON.stringify({
            type: 'activity',
            action: 'CORRECTION_POLICY_UPDATE',
            details: { title },
            user: 'Admin'
          })
        });
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[300px] gap-2">
        ...
        <span className="text-xs font-semibold text-slate-500 font-sans">Syncing Correction Policy configuration...</span>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-[1150px] animate-[admin-fade-in_0.4s_ease_both] pb-12">
      <div className="flex justify-between items-start mb-6">
        <div>
          <h1 className="text-[22px] font-sans font-extrabold text-[#111] m-0">Correction Policy Manager</h1>
          <p className="text-[12.5px] text-slate-500 mt-1 font-medium">Manage editorial accuracy standards and feedback channels dynamically.</p>
        </div>
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-[0_4px_20px_rgba(0,0,0,0.01)] space-y-4">
            ...
            {sections.length === 0 ? (
              <div className="text-center text-slate-400 text-xs py-8 font-sans border border-dashed border-slate-200 rounded-lg">
                No policy sections added yet. Click "+ Add Standards Section".
              </div>
            ) : (
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-[0_4px_20px_rgba(0,0,0,0.01)] sticky top-6 space-y-4">
            ...
            <div className="border border-slate-150 rounded-lg p-4 bg-white font-sans max-h-[600px] overflow-y-auto space-y-4 select-none">
              <div className="pb-3 border-b border-slate-100">
                <h2 className="font-serif text-lg font-bold text-slate-900 leading-tight">{title || 'Correction Policy'}</h2>
                <p className="text-[9.5px] font-bold text-slate-400 uppercase mt-0.5 tracking-wider">{subtitle || 'Commitment to Accuracy and Integrity'}</p>
              </div>
